...
####################
# This script is used to remove snap packages.
####################
...
SNAP_PACKAGES_FILE="../text/snap_packages_to_remove.txt"
...
# Inform the user which snap packages will be removed
echo ""
echo "Removing the following snap packages:"
echo "---------------------------------"
...
read -rp "This script will remove snap packages. Did you install snapd and run snap refresh first? (Y/n) " -n 1
echo ""
if [[ ! "${REPLY}" =~ ^[Yy]$ ]]
then
...
# Remove each snap package listed in the txt file line by line
for PACKAGE in "${SNAP_PACKAGES[@]}"; do
